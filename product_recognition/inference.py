"""Backend-facing product recognizer and nearest-neighbor prediction logic.
...
def load_thresholds(
    ...
    Input:
        threshold_dir: Folder containing threshold optimization outputs, or None.
        default_score_threshold: Fallback threshold when no threshold folder is used.
    Output:
    ...
    threshold_path = Path(threshold_dir)
    threshold_table_path = threshold_path / "thresholds.csv"
...
def load_recognizer(
    ...
    Input:
        cache_dir: Folder containing the memory-bank pickle.
        cache_name: Cache filename or filename without `.pkl`.
        threshold_dir: Optional folder with threshold CSV outputs.
    ...
    if memory_embeddings is None:
        raise ValueError(
            "Memory-bank cache does not contain memory_embeddings. "
            "Create the cache with python -m product_recognition.cli.create_memory_bank before running inference."
        )
    ...
    # Thresholds are optional for local runs without an optimization folder.
    # In deployed inference, pass `threshold_dir` to use calibrated thresholds.
    score_threshold, class_thresholds = load_thresholds(threshold_dir)
